...
/*-------------------------------------------------------------------
 * This function handles the membership service (from the consumer)
 * if it offers this LTI extension
 ------------------------------------------------------------------*/
function lti_sync_enrolments() {
  ...
  if (empty($_SESSION[LTI_SESSION_PREFIX . 'all']))          $_SESSION[LTI_SESSION_PREFIX . 'all'] = array();
  if (empty($_SESSION[LTI_SESSION_PREFIX . 'provision']))    $_SESSION[LTI_SESSION_PREFIX . 'provision'] = array();
  if (empty($_SESSION[LTI_SESSION_PREFIX . 'new_to_blog']))  $_SESSION[LTI_SESSION_PREFIX . 'new_to_blog'] = array();
  if (empty($_SESSION[LTI_SESSION_PREFIX . 'newadmins']))    $_SESSION[LTI_SESSION_PREFIX . 'newadmins'] = array();
  if (empty($_SESSION[LTI_SESSION_PREFIX . 'changed']))      $_SESSION[LTI_SESSION_PREFIX . 'changed'] = array();
  if (empty($_SESSION[LTI_SESSION_PREFIX . 'role_changed'])) $_SESSION[LTI_SESSION_PREFIX . 'role_changed'] = array();
  if (empty($_SESSION[LTI_SESSION_PREFIX . 'remove']))       $_SESSION[LTI_SESSION_PREFIX . 'remove'] = array();
  if (empty($_SESSION[LTI_SESSION_PREFIX . 'nochanges']))    $_SESSION[LTI_SESSION_PREFIX . 'nochanges'] = 0;
  ...
  case 'continue':
    ...
    $lti_users = $resource_link->doMembershipsService();

    if (!$lti_users) {
      echo '<h2>' . __('Error on Synchronisation', 'lti-text') . '</h2>';
      echo '<p>' . sprintf(__('Error returned from consumer (%s)', 'lti-text'), $consumer->name) . '</p>';
      echo '<p>' . sprintf(__('Request to consumer:  %s', 'lti-text'), '<br />' . urldecode(str_replace('&', '<br />', $resource_link->ext_request))) . '</p>';
      echo '<p>' . sprintf(__('Response from consumer: %s', 'lti-text'), '<br />' . $resource_link->ext_response) . '</p>';
      return;
    }
    // Get straight-forward list to work with in WordPress
    $membership_consumer = lti_create_user_list($lti_users);
    ...
    $membership_consumer = lti_create_membership_lists($membership_consumer);

    // Assign list to $_SESSION variables
    $_SESSION[LTI_SESSION_PREFIX . 'all']          = serialize($membership_consumer);
    $_SESSION[LTI_SESSION_PREFIX . 'provision']    = serialize(lti_get_members($_SESSION[LTI_SESSION_PREFIX . 'all'], 'provision'));
    $_SESSION[LTI_SESSION_PREFIX . 'new_to_blog']  = serialize(lti_get_members($_SESSION[LTI_SESSION_PREFIX . 'all'], 'new_to_blog'));
    $_SESSION[LTI_SESSION_PREFIX . 'newadmins']    = serialize(lti_get_members($_SESSION[LTI_SESSION_PREFIX . 'all'], 'newadmins'));
    $_SESSION[LTI_SESSION_PREFIX . 'changed']      = serialize(lti_get_members($_SESSION[LTI_SESSION_PREFIX . 'all'], 'changed'));
    $_SESSION[LTI_SESSION_PREFIX . 'role_changed'] = serialize(lti_get_members($_SESSION[LTI_SESSION_PREFIX . 'all'], 'rchanged'));

    // Blog members for removal need to be handled differently
    $_SESSION[LTI_SESSION_PREFIX . 'remove'] = serialize(lti_deleted_members($membership_consumer, $current_members));

  case 'all': // Not currently used
    // Display all the members from the consumer
  ...
  case 'provision': // Not currently used
    ...
    if (!empty($_SESSION[LTI_SESSION_PREFIX . 'provision'])) lti_display($_SESSION[LTI_SESSION_PREFIX . 'provision'], $ltiuser);
    break;
  ...
  case 'new_to_blog': // Not currently used
    ...
    if (!empty($_SESSION[LTI_SESSION_PREFIX . 'new_to_blog'])) lti_display($_SESSION[LTI_SESSION_PREFIX . 'new_to_blog'], $ltiuser);
    break;

  case 'newadmins':
    echo "<h2>" . __('Membership Synchronisation - New Administrators to Blog', 'lti-text') . "</h2>";
    if (!empty($_SESSION[LTI_SESSION_PREFIX . 'newadmins'])) lti_display($_SESSION[LTI_SESSION_PREFIX . 'newadmins'], $ltiuser);
    break;

  case 'changed': // Not currently used
    echo "<h2>" . __('Membership Synchronisation - Changed Members', 'lti-text') . "</h2>";
    if (!empty($_SESSION[LTI_SESSION_PREFIX . 'changed'])) lti_display($_SESSION[LTI_SESSION_PREFIX . 'changed'], $ltiuser);
    break;

  case 'rchanged':
    echo "<h2>" . __('Membership Synchronisation - Role Changed', 'lti-text') . "</h2>";
    if (!empty($_SESSION[LTI_SESSION_PREFIX . 'role_changed'])) lti_display($_SESSION[LTI_SESSION_PREFIX . 'role_changed'], $ltiuser);
    break;

  case 'remove':
    echo "<h2>" . __('Membership Synchronisation - Members for Removal from Blog', 'lti-text') . "</h2>";
    if (!empty($_SESSION[LTI_SESSION_PREFIX . 'remove'])) lti_display($_SESSION[LTI_SESSION_PREFIX . 'remove'], $ltiuser);
    break;

  case 'error':
    echo "<h2>" . __('Synchronisation Errors', 'lti-text') . "</h2>";
    echo "<p>" . $_SESSION[LTI_SESSION_PREFIX . 'error'] . "</p>";
    echo "<p>" . __('Remaining users from consumer added', 'lti-text') . "</p>";
    $_SESSION[LTI_SESSION_PREFIX . 'error'] = '';
    break;

  default:

    // If consumer has setting service then get date/time of last synchronisation
    $last_sync = '';
    if ($resource_link->hasSettingService()) {
        $last_sync = $resource_link->doSettingService(LTI_Resource_Link::EXT_READ, NULL);
    }
  ...
  <p><?php _e('Click on the <i>Continue</i> to obtain a list of the changes to be processed. The updates
   ...
   <?php if (!empty($last_sync)) echo '<p>' . sprintf(__('Last Synchronisation: %s', 'lti-text'), $last_sync) . '</p>'; ?>
