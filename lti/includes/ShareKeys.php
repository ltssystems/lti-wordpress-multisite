...
function lti_create_share_key() {
  ...
  if (!empty($_POST['email'])) {
    ...
    if (isset($_POST['enabled'])) $_POST['enabled'] ? $share_key->auto_approve = TRUE : $share_key->auto_approve = FALSE;

    $share_key->save();

    wp_get_current_user();
    $senttext = __('Instructor: ', 'lti-text') . '<b>' . $current_user->display_name . '</b>' . __(' has shared', 'lti-text') . '<br /><br />' .
                __('Blog Name: ', 'lti-text') . '<b>' . get_bloginfo('name') . '</b>' . __(' with your module. To link up with this Blog:', 'lti-text') . '<br /><br />' .
                sprintf(__('Place this key (%s) in the custom parameters of the link to WordPress in your course as:', 'lti-text'), $share_key->getId()) . '<br /><br />' .
                sprintf(__('share_key=%s', 'lti-text'), $share_key->getId());

    // Write friendly times
    if ($_POST['life'] <=  12) $time = sprintf(_n('%s hour', '%s hours', $_POST['life'], 'lti-text'), $_POST['life']);
    if ($_POST['life'] >=  24 && $_POST['life'] <= 144) {
      $days = intval($_POST['life']/24);
      $time = sprintf(_n('%s day', '%s days', $days, 'lti-text'), $days);
    }
    if ($_POST['life'] == 168) $time = '1 week';
  ...
  <?php if (lti_get_scope($_SESSION[LTI_SESSION_PREFIX . 'key']) == 0) {
    echo
    '<p><strong>' .
    __('The username format of this tool consumer is set to global and it is NOT recommended to share your site when user accounts are being created using this option.', 'lti-text') .
    '</strong></p>';
  }
  ?>
          ...
          <td>
            <fieldset>
              ...
              <label for="enabled">
                ...
                <?php _e('Automatically allow requests from this share without further approval', 'lti-text'); ?>
              </label>
            </fieldset>
          </td>
        </tr>
...
function lti_manage_share_keys_screen_options() {
  ...
  $screen->add_help_tab( array(
    ...
    'content' => '<p>' . __('You can customise the display of this screen by specifying the number of share keys to be displayed per page.', 'lti-text') . '</p>'
  ));
}
?>