...
/**
 *  List of supported message types and associated callback methods
 */
...
// echo "callback = $callback<br>" . $this->callbackHandler[$callback] . "<br>";
...
/*  // DEBUG for development only	 
 echo "Redirec: " . $this->redirectURL . "<br>"; 
 echo "<h5>sessions=</h5><br>";
 print_r($_SESSION);
	  exit;
*/
...
// echo "Check here again!<br>";		
 //       $this->resource_link = new LTI_Resource_Link($this->consumer, trim($_POST['resource_link_id']));  // UPDATED BY W PAN
         $this->resource_link = new LTI_Resource_Link($this->consumer, trim($_POST['user_id']));
        if (isset($_POST['context_id'])) {
          $this->resource_link->lti_context_id = trim($_POST['context_id']);
        }
 //       $this->resource_link->lti_resource_id = trim($_POST['resource_link_id']);    //     UPDATED BY W PAN
        $this->resource_link->lti_resource_id = trim($_POST['user_id']);

//     UPDATED BY W PAN
        $title = $_POST['lis_person_name_full'] . "'s Blog Website";
// this block is replaced by the above line 	  
 /*     $title = '';
...
// echo "this->resource_link=<br>"; print_r($this->resource_link); exit;
        $this->user = new LTI_User($this->resource_link, $user_id);
// echo "this->user=<br>"; print_r($this->user);
#
### Set the user name
#
...
#
        ...
        //   $this->user->roles = LTI_Tool_Provider::parseRoles($_POST['roles']);   //     UPDATED BY W PAN
	       $this->user->roles = LTI_Tool_Provider::parseRoles('Administrator');   //     maybe need further chnage
        }
#
...
// echo "<h5>PB key=$key</h5>"; echo "<h5> SERVER['REQUEST_URI']=" . $_SERVER['REQUEST_URI'] . "</h5>"; 
// echo "<br>++++ Path +++++<br>";  debug_print_backtrace(); echo "<br>+++++++++<br>";
